...
public class Main extends ApplicationAdapter implements InputProcessor{
	...
	public void render () {
		...
		if(SI.getButtonClicked()) {
				...
				firstFrameOfSwing = false;
				p.moveBall(SI.getDir() + angleIncrease, SI.getSpd());



		}

		if(AI.getButtonClicked() || aiGoing==true)
		{
			aiGoing = true;
			if(firstFrameOfSwing){
				p.setBallBlockedX(false);
			...
			firstFrameOfSwing = false;
			p.moveBall(AI.getDir()+angleIncrease, AI.getSpd());
        ...
        */
		...
		if(water.x - golfBall.x <= 33 && water.x - golfBall.x >= -110 && water.y - golfBall.y <= 33 && water.y - golfBall.y >= -70){
			...
			System.out.println(" u in water. respawn.");
			golfBall.x = p.positionX();
			golfBall.y = p.positionY();
			p.golfBall.setVX2(0);
			p.golfBall.setVY2(0);
