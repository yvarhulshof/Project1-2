...
public class Main extends ApplicationAdapter implements InputProcessor{
	...
	private float goalX;
	private float goalY;
	private float goalXFF;
	private float goalYFF;
	DijkstraMain DM;
	...
	public void create () {
		...
		//tiledMap = new TmxMapLoader().load("mapMaze1.tmx"); // maze like course for the AI
		tiledMapRenderer = new OrthogonalTiledMapRenderer(tiledMap); //creates the background map (visual)


		water = new Rectangle(350, 175, 160, 80);
       ...
       /* mapInfo = FI.readMapInfo(); //receiving information about the map (non-visual, physics related) and then setting these values in the physics engine
		...
		DM = new DijkstraMain(collisionLayer);
	...
	public void render () {
		...
		if (AI.getVisible()) {
			batch.draw(golfballImg, golfBalls[cpp].x, golfBalls[cpp].y); //draws the golfBall image at the location where the golfBall Circle object is at that point in time
		} //draws the golfBall image at the location where the golfBall Circle object is at that point in time
		batch.end();
		...
		/** calls the AI methods for the angle, the direction, adjust its speed after each try */
		if (AI.getButtonClicked() || aiGoing) {
			...
			if(shotIndex == midpoints.get(0).size()-1){
				...
				goalX = midpoints.get(0).get(shotIndex); // get the x location for the midpoint goal
				goalY = midpoints.get(1).get(shotIndex); // get the y location for the midpoint goal
			}
			...
			if (p[cpp].firstAICall) {
				...
				//System.out.println("shot w/ destination " + goalX + " " + goalY);
				//System.out.println("position of the ball: x : " + golfBalls[cpp].getXCoords() + " y " + golfBalls[cpp].y);
			}
			...
			if(p[cpp].getBallStopped()&& midPointFound){
				golfBalls[cpp].x = goalXFF;
				golfBalls[cpp].y = goalYFF ;
				speedIncrease += 100;
			}
