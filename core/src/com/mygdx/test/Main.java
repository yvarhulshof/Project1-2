...
public class Main extends ApplicationAdapter implements InputProcessor{
	...
	public void create () {
        ...
        cpp = 0;
			...
			GolfBall golfBall = new GolfBall(0,0);
			PhysicsEngine p2 = new PhysicsEngine(golfBall,collisionLayer);
	...
	public void render () {
		/*
		if(golfBalls[cpp].x == 0){
			golfBalls[cpp].x = 32;
		}
		if(golfBalls[cpp].y == 0){
			golfBalls[cpp].y = 32;
		}
		*/
...
//			float ballY = golfBalls[cpp].getYCoords();
			...
			*/
			...
			if (p2.firstAICall) {
				...
				aiDirection = p2.aiAngle(ballX, ballY, goalX, goalY);
			}


				//if(Math.abs(golfBalls[cpp].x - goalX) < 5 && Math.abs(golfBalls[cpp].x - goalX) < 5 && firstFrameOfSwing){
			if(Math.abs(golfBall.x - goalX) < 5 && Math.abs(golfBall.y - goalY) < 5 && firstFrameOfSwing){
			//if(firstFrameOfSwing){
					System.out.println("shot with goal: " + goalX + " " + goalY);
					//p[cpp].setBallBlockedX(false);
				p2.setBallBlockedX(false);
				//p[cpp].setBallBlockedY(false);
				p2.setBallBlockedY(false);
					firstFrameOfSwing = false;
					shotIndex++;
					//TODO change aiDirection to be just goalsDirection, let speed depend on distance
					double nextAngle = p[cpp].findNextAngleDijkstra(ballX ,ballX ,goalX ,goalY);
					//p[cpp].moveBall(nextAngle, 500);
				p2.moveBall(nextAngle, 500);
